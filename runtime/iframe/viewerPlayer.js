...
Shumway.BinaryFileReader.prototype.readAsync = function (ondata, onerror, onopen, oncomplete, onhttpstatus) {
  ...
  console.log("AIR runtime: fetching", this.url);
  fetch(this.url, { method: this.method || "GET" })
    .then(function (response) {
      console.log("AIR runtime: response", response.status, reader.url);
      if (onhttpstatus) {
        onhttpstatus(reader.url, response.status, "");
      }
      if (!response.ok) {
        throw new Error("HTTP " + response.status + " while loading " + reader.url);
      }
      return response.arrayBuffer();
    })
    .then(function (buffer) {
      console.log("AIR runtime: received", buffer.byteLength, "bytes");
    ...
    .catch(function (error) {
      console.error("AIR runtime: fetch failed", error);
      if (onerror) {
